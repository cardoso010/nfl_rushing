defmodule NflRushing.StatisticsTest do
  ...
  describe "football_players" do
    ...
    @valid_attrs %{att: 120.5, att_g: 120.5, avg: 120.5, first_down: 120.5, first_down_porcent: 120.5, forty_plus: 120.5, fum: 120.5, lng: "some lng", player: "some player", pos: "some pos", td: 120.5, team: "some team", twenty_plus: 120.5, yds: 120.5, yds_g: 120.5}
    ...
    test "create_football_player/1 with valid data creates a football_player" do
      assert {:ok, %FootballPlayer{} = football_player} = Statistics.create_football_player(@valid_attrs)
    ...
    test "update_football_player/2 with valid data updates the football_player" do
      football_player = football_player_fixture()
      assert {:ok, %FootballPlayer{} = football_player} = Statistics.update_football_player(football_player, @update_attrs)
    ...
    test "update_football_player/2 with invalid data returns error changeset" do
      football_player = football_player_fixture()
      assert {:error, %Ecto.Changeset{}} = Statistics.update_football_player(football_player, @invalid_attrs)
    ...
    test "delete_football_player/1 deletes the football_player" do
      ...
      assert_raise Ecto.NoResultsError, fn -> Statistics.get_football_player!(football_player.id) end
    end
